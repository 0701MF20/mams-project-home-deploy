...
const AppHeader = () => {
  ...
  const [isSmallScreen, setIsSmallScreen] = useState(window.innerWidth < 685)

  //new addition::::
  const colorModes = useSelector((state) => state.colorModes)
  const handleSetColorMode = (mode) => {
    dispatch({ type: 'setColorModes', colorMode: mode })
  }
  useEffect(() => {
    const handleResize = () => {
      ...
      <CContainer className="border-bottom px-4" fluid>
        ...
        <CHeaderNav className="ms-auto d-flex align-items-center">
          <CNavItem>
            ...
            >
              {/* Meter Allocation & Monitoring System */}
              {/* METER ALLOCATION & MONITORING SYSTEM */}
           <strong>METER ALLOCATION & MONITORING SYSTEM</strong>  
            </span>
            <span
              style={{
